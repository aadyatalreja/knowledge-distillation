...
def main():
    class Args:
        ...
        b = BATCH_SIZE
        lr = LEARNING_RATE
        teacher = '/content/best_model_vgg16.pth'
        student = '/content/best_model_mobilenetv2.pth'
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if acc > best_acc:
            ...
            best_state_dict = student.state_dict().copy()
            early_stopping_counter = 0  # Reset counter when we have improvement
        else:
            ...
            if early_stopping_counter >= EARLY_STOPPING_PATIENCE:
                ...
                final_model_path = os.path.join(args.output_dir, f'kd_mobilenet-{epoch}-final.pth')
                torch.save(student.state_dict(), final_model_path)
                # Also save the best model we found during training
                if best_state_dict is not None:
                    best_overall_path = os.path.join(args.output_dir, 'kd_mobilenet-best-overall.pth')
                    torch.save(best_state_dict, best_overall_path)
                    print(f'Saved best overall model to {best_overall_path}')
                break
